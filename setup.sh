...
cp "$SCRIPT_DIR/config/nginx/"* /etc/nginx/conf.d/
systemctl restart nginx

# === Copy XRAY Configs ===
echo -e "\e[92m[INFO] Copying Xray configs...\e[0m"
mkdir -p /etc/xray
cp -r "$SCRIPT_DIR/config/xray-template/"* /etc/xray/

# === Setup Telegram Bot ===
read -rp "Masukkan TOKEN BOT TELEGRAM: " BOT_TOKEN
read -rp "Masukkan CHAT_ID BOT: " BOT_CHAT_ID

sed -i "s|BOT_TOKEN=.*|BOT_TOKEN='$BOT_TOKEN'|" "$SCRIPT_DIR/bot/telegram-bot.py"
sed -i "s|BOT_CHAT_ID=.*|BOT_CHAT_ID='$BOT_CHAT_ID'|" "$SCRIPT_DIR/bot/telegram-bot.py"
cp "$SCRIPT_DIR/bot/telegram-bot.py" /usr/bin/telegram-bot.py
